...
| **调用关系** | 不知道 | 知道谁调用谁（`callers` / `callees` / `graph path`） |
| **影响分析** | 不可能 | `graph impact` 告诉你改一个函数会影响多少地方 |
| **Token 开销** | 读整个文件（~4000 tokens） | 只返回签名（~60 tokens）—— **节省 66 倍** |
| **引用查找** | `grep "foo"` 会匹配注释、字符串里的 foo | AST 级别，只匹配真正的代码引用 |
