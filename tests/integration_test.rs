...
function bar(): void {}
    ...
    #[test]
    fn vue_parsed_output_includes_identifiers() {
        let parser = symlens::parser::vue::VueParser;
        let source = include_bytes!("fixtures/sample.vue");
        let output = symlens::parser::traits::LanguageParser::extract_all(
            &parser,
            source,
            std::path::Path::new("sample.vue"),
        )
        .expect("Failed to parse Vue file");
        assert!(
            !output.identifiers.is_empty(),
            "Vue extract_all should return identifiers, got 0"
        );
    }
